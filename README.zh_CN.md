...
| `ALIYUN_SMS_ACCESS_KEY_ID` | 阿里云短信 AccessKey ID；启用手机注册验证时必填，且必须配置共享 Redis | - |
| `ALIYUN_SMS_ACCESS_KEY_SECRET` | 阿里云短信 AccessKey Secret；启用手机注册验证时必填 | - |
| `ALIYUN_SMS_SIGN_NAME` | 已审核通过的阿里云短信签名 | - |
| `ALIYUN_SMS_TEMPLATE_CODE` | 已审核通过的验证码短信模板 Code，模板参数需包含 `code` | - |
